...
/* globals L */
L.Control.BuSearch = L.Control.extend({
    options: {
        position: 'topleft',
        lazy_mode: false,
    },
    ...
    onAdd: function (map) {
        "use strict";
        this.map = map;
        var marker_container = L.DomUtil.create('div', 'bu-search-container');
    ...
    onMapClickAddMarker: function (e) {
        ...
        if(this.options.drag_marker_callback){
            marker.addEventListener("dragend", this.options.drag_marker_callback);
        }
    ...
    onMarkerClickDelete: function (e) {
        ...
        if (this.markerList.indexOf(e.target)) {
            if (this.options.remove_marker_callback) {
                console.log("remove callback");
                this.options.remove_marker_callback(e.target);
            }
